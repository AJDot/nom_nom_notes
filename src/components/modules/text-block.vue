<template>
  <draggable :draggable="draggable" :droppable="droppableTest" class="relative basis-full rounded-md" :item="block" :list-id="block.parentId ?? ''" @drop="onDrop">
    <div :key="block.id" :placeholder="placeholder" data-focus class="text px-2 py-1 cursor-text outline-none border-2 border-transparent rounded-md break-anywhere focus:shadow-input focus:bg-gray-100 after:text-gray-500 after:empty:content-[attr(placeholder)]" :contenteditable="isEditable" v-html="block.content.text" ref="content"></div>
  </draggable>
</template>
