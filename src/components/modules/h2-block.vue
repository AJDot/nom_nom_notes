<template>
  <draggable :draggable="draggable" :droppable="droppable" class="relative basis-full rounded-md" :item="block" :list-id="block.parentId ?? ''" @drop="onDrop">
    <h2 class="text-2xl px-2 py-1 cursor-text outline-none border-2 border-transparent focus:shadow-input  focus:bg-gray-100 rounded-md" contenteditable data-focus ref="content" v-html="block.content.text"></h2>
  </draggable>
</template>
