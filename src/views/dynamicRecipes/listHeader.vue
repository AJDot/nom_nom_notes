<template>
  <ul class="flex gap-5 place-items-center text-2xl mt-4">
    <li data-test="new-dynamic-recipe-group" class="flex items-center relative">
      ...
      <dropdown v-if="signedIn" :state="newDynamicRecipeDropdownState" @close="closeNewDynamicRecipeDropdown" position-type="mouse">
        <template #control>
          <button @click="toggleNewDynamicRecipeDropdown" type="button" data-test="new-dynamic-recipe-dropdown-button" class="flex items-center outline-none transition-transform active:shadow-inner hover:text-green" :class="{ 'rotate-180': newDynamicRecipeDropdownState }">
  ...
  <Modal :state="modalState" center width="lg">
    <template #header>
      ...
      <div class="flex">
        <section class="basis-1/4">
          <ul class="w-full pr-2 border-r border-r-gray-300">
            <li v-for="template in templates" class="w-full py-2 px-1 border-y border-y-gray-300 hover:bg-blue-100" :class="isTemplateSelected(template) ? 'bg-blue-300 hover:bg-blue-300' : ''">
              <button type="button" class="text-left w-full h-full" @click="selectedTemplate = template">
                <p>{{ template.name }}</p>
        ...
        <section class="grow overflow-auto">
          <template v-if="selectedTemplate">
            <base-block-group :blocks="selectedTemplate.blocks.filter(b => !b.parentId)" :director="new BlockDirector({ blocks: selectedTemplate.blocks })" />
          </template>
        </section>
      </div>
    </template>
    <template #footer>
      <button class="btn ml-3 text-white bg-green hover:text-white hover:bg-green-700 disabled:bg-green-100" type="button" @click="startDynamicRecipe" :disabled="!Boolean(selectedTemplate)">
        Select
      </button>
      <button class="btn ml-3 text-gray-900 bg-white border-solid border border-gray-400 hover:bg-gray-100 active:bg-gray-100 focus:bg-gray-100" @click="resetSelectTemplateModal">
        Cancel
      </button>
    </template>
  </Modal>
...
interface Data {
  ...
  BlockDirector: typeof BlockDirector
}
